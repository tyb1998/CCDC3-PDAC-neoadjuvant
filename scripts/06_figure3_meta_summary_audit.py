#!/usr/bin/env python3
"""Validate Figure 3 meta-analysis result tables and internal arithmetic.

This script audits retained cohort estimates and published summary tables. It
does not refit cohort Cox models because expression/clinical objects were not
included in the source-data archive.
"""
import argparse
...
def fixed_effect(rows):
    ...
    # The retained tables include rounded CIs; reconstruct SE from both bounds.
...
def main():
    ...
    audit.to_csv(args.output / 'figure3_fixed_effect_audit.csv', index=False)
